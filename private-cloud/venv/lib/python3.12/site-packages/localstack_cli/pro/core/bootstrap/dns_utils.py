...
def configure_systemd(revert:bool):
	I='resolvectl';H='sudo';A=False;from subprocess import CalledProcessError as K;from localstack_cli import config as D;from localstack_cli.utils.container_utils.container_client import ContainerException as L;from localstack_cli.utils.docker_utils import DOCKER_CLIENT as E;from localstack_cli.utils.run import is_linux as M,run as F,to_str as N
	if not M():LOG.warning('Command only supported on GNU/Linux')
	try:
		O=E.get_container_ip(D.MAIN_CONTAINER_NAME);G=E.get_networks(D.MAIN_CONTAINER_NAME);G=G[0];J=E.inspect_network(G);B=J['Options'].get('com.docker.network.bridge.name');P=J['Id'][:12];B=B or f"br-{P}"
	...
	except K as Q:LOG.warning('Error while configuring systemd-resolved: %s',N(Q.output).strip())