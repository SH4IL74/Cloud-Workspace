...
def set_token(auth_token:str):
	from localstack_cli.pro.core.bootstrap.licensingv2 import AuthToken as D;A=D(auth_token)
	...
	from localstack_cli.pro.core import config as E;from localstack_cli.pro.core.bootstrap.auth import get_auth_cache as F
...
@publish_invocation
def clear_token():
	from localstack_cli.pro.core import config as D;from localstack_cli.pro.core.bootstrap.auth import get_auth_cache as E;from localstack_cli.pro.core.bootstrap.licensingv2 import ENV_LOCALSTACK_AUTH_TOKEN as B,AuthToken as F
	try:
		A=E()
...
def show_token(plain:bool):
	H='Token not configured in environment yet, please run localstack auth set-token <AUTH_TOKEN>, or set the environment variable LOCALSTACK_AUTH_TOKEN to a valid auth token. You can find your auth token in the LocalStack web app https://app.localstack.cloud.';from localstack_cli.pro.core import config as C;from localstack_cli.pro.core.bootstrap.auth import get_auth_cache as I;from localstack_cli.pro.core.bootstrap.licensingv2 import ENV_LOCALSTACK_AUTH_TOKEN as A,AuthToken as B
	def D(_token:B):
		A=_token;B=A.is_valid();click.echo(f"Valid: {B}",err=_A)
		if plain:click.echo(A.token)
		else:click.echo(f"Token: {A}")
	E=os.getenv(A,'').strip()
	if E:click.echo(f"Prioritizing auth token set in environment variable {A}",err=_A);D(B(E));return
	if not os.path.isfile(C.AUTH_CACHE_PATH):click.echo(H,err=_A);return
	try:J=I()
	except Exception as F:raise ClickException(f"Could not load auth configuration from {C.AUTH_CACHE_PATH}: {F}")from F
	G=J.get(A,'')
...
@publish_invocation
def logout()->None:from localstack_cli.pro.core.bootstrap import auth;auth.logout();click.echo('successfully logged out')