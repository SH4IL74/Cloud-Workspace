...
_B=True
_A=None
import base64,copy,dataclasses,hmac,json,logging,os,platform,re,textwrap,threading
...
from localstack_cli.pro.core.bootstrap.entitlements import ProductEntitlements,ProductInfo
...
class LicenseV1Client(LicenseV1ClientBase):
	...
	def _get_machine_data(C)->dict:from localstack_cli.utils.analytics.metadata import get_client_metadata as B;A=B();return{'id':A.machine_id,'cli':config.is_env_true(_G),'ci':A.is_ci,'system':get_system_information_summary()}
	def _get_product_data(B)->dict:from localstack_cli.pro.core.constants import VERSION as A;return{_I:'localstack-pro',_E:A}
	def _perform_licensing_request(E,path:str,payload:dict):
		import requests as A;from localstack_cli.utils.http import get_proxies as B;from localstack_cli.utils.sync import retry;C=B()
		def D():
...
class LicensedLocalstackEnvironment:
	...
	def activate(A,offline_only:bool=_C):
		with A._mutex:
			...
			A.activate_license(offline_only);os.environ[constants.ENV_PRO_ACTIVATED]='1'
...
def get_system_information_summary()->str:
	C=','
	try:from localstack_cli.utils.docker_utils import DOCKER_CLIENT as D;B=D.get_system_info();return C.join([B['OperatingSystem'],B['KernelVersion'],B['Architecture']])
...
def get_credentials_from_environment()->Credentials|_A:
	...
	if not A and config.is_env_true(_G):
		from localstack_cli.pro.core.bootstrap.auth import get_auth_cache as D
		try:A=D().get(_J)
...
def configure_container_licensing(cfg):
	B=cfg;from localstack_cli.utils.container_utils.container_client import BindMount as D;A=get_credentials_from_environment()
